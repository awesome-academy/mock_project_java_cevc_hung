...
public class TourImportExportService {
    ...
    public String exportToursToCSV() {
        ...
        for (TourEntity tour : tours) {
            ...
            csv.append(escapeCsv(tour.getSeatsAvailable() != null ? tour.getSeatsAvailable().toString() : "")).append(",");
            csv.append(escapeCsv(tour.getStartDate() != null ? tour.getStartDate().format(DATE_FORMATTER) : "")).append(",");
            csv.append(escapeCsv(tour.getEndDate() != null ? tour.getEndDate().format(DATE_FORMATTER) : "")).append(",");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    CategoryEntity category = categoryRepository.findAll().stream()
                            .filter(c -> c.getName().equalsIgnoreCase(importRequest.getCategoryName().trim()))
                            .findFirst()
                            .orElse(null);
                    ...
                    TourEntity tour = TourEntity.builder()
                            ...
                            .seatsAvailable(importRequest.getSeatsAvailable() != null ? 
                                    importRequest.getSeatsAvailable() : importRequest.getSeatsTotal())
    ...
    private TourImportRequest parseCsvLine(String line, int rowNumber) throws Exception {
        ...
        if (values.size() < 13) {
            throw new Exception("Invalid CSV format: expected 13 columns, found " + values.size());
        }
        ...
        try {
            ...
            request.setStartDate(values.get(8).trim().isEmpty() ? null : LocalDateTime.parse(values.get(8).trim(), DATE_FORMATTER));
            request.setEndDate(values.get(9).trim().isEmpty() ? null : LocalDateTime.parse(values.get(9).trim(), DATE_FORMATTER));
